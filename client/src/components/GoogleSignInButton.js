...
// Inline 4-color Google "G" (vector — not an image asset). Official multi-path
// version so it renders crisply at any size and respects the no-image constraint.
...
const GoogleSignInButton = ({ onClick, loading = false, disabled = false, label = "Continue with Google", sx }) => (
  <Button
    ...
    startIcon={!loading ? <GoogleG /> : undefined}
    sx={{
      borderRadius: 2,
      py: 1.35,
      borderColor: "divider",
